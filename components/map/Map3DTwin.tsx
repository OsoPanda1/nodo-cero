...
/* Portado de visitarealdelmonte (Map3DTwin.tsx). Terreno 3D con       */
/* shader de niebla, órbita libre y marcadores reales de RDM_POIS.     */
/* Degrada a una lista de nodos cuando WebGL no está disponible.       */
/* ------------------------------------------------------------------ */
...
const FOG_UNIFORMS = { uTime: { value: 0 } };
...
function FogPlane() {
  ...
  return (
    <mesh ref={meshRef} position={[0, 1.25, 0]} rotation-x={-Math.PI / 2}>
...
function Atmosphere() {
  return (
    <>
      <fogExp2 attach="fog" args={["#0b1323", 0.055]} />
...
export function Map3DTwin({ viewport, markers }: Map3DTwinProps) {
  ...
  useEffect(() => {
    /* La detección de WebGL es exclusiva del cliente (document/contexto). */
  ...
  if (!webglReady) {
    return (
      <div className="relative flex h-[420px] w-full flex-col justify-between overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-[#0a1222] via-[#101a2f] to-[#0a0f1f] p-5 md:h-[640px]">
        <div>
          <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Modo híbrido degradado</p>
          <h3 className="mt-2 text-xl font-semibold text-slate-100">
            Visualización 3D no disponible en este entorno
          </h3>
          <p className="mt-2 max-w-2xl text-sm text-slate-400">
            Se mantiene un mapa de nodos para equipos sin WebGL habilitado.
          </p>
        </div>
  ...
  return (
    <div className="relative h-[420px] w-full overflow-hidden rounded-2xl border border-white/10 bg-[#070b14] md:h-[640px]">
      ...
      <div className="absolute bottom-3 left-3 z-20 rounded-lg border border-white/15 bg-slate-950/75 px-3 py-2 text-xs text-slate-300 backdrop-blur-sm">
